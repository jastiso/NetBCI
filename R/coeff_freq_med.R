...
# load data
med = readMat(paste('data/med_freq_coeff.mat'))
...
for (b in bands){
  ...
  plot_data[cnt:(cnt+nSubj-1),3] = med$med.high[,b_idx]
  cnt = cnt + nSubj
  
  plot_data[cnt:(cnt+nSubj-1),1] = rep('other BE', times = nSubj)
  plot_data[cnt:(cnt+nSubj-1),2] = rep(b, times = nSubj)
  plot_data[cnt:(cnt+nSubj-1),3] = med$med.other[,b_idx]
  cnt = cnt + nSubj
  
  
  plot_data[cnt:(cnt+nSubj-1),1] = rep('low BE', times = nSubj)
  plot_data[cnt:(cnt+nSubj-1),2] = rep(b, times = nSubj)
  plot_data[cnt:(cnt+nSubj-1),3] = med$med.low[,b_idx]
  cnt = cnt + nSubj
...
plot + geom_boxplot(notch = FALSE, lwd = 1) + 
  ...
  labs(x = 'Subgraph', y = 'Coeff Median')  + theme_minimal()
ggsave(paste('med_coeff_freq', '.png', sep = ''))
