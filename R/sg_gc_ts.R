...
#############################################################################################
S = readMat(paste('data/gc/', sens, '/Ssg_ts.mat', sep = ''))
...
ggsave(paste(sens, '_Ssg_gc_ts', '.png', sep = ''))
...
#############################################################################################
# load data
Sp = readMat(paste('data/gc/', sens, '/Sp_ts.mat', sep = ''))
...
plot + geom_boxplot(notch = FALSE, lwd = 1) + 
  scale_color_manual(values = c(rgb(0,0,0, alpha = 1), rgb(.3,.3,.3, alpha = 1), rgb(.6,.6,.6, alpha = 1))) + 
  scale_fill_manual(values = rev(brewer.pal(4,"Greys"))) + 
  labs(x = 'Subgraph', y = 'Sparsity')  + theme_minimal()
ggsave(paste(sens, '_Sp_gc_ts', '.png', sep = ''))
