...
h = readMat(paste('data/wpli/', sens, '/mod_exp_high.mat', sep = ''))
l = readMat(paste('data/wpli/', sens, '/mod_exp_low.mat', sep = ''))

high = data.frame(subj = character(len = length(h$band.h)), band = character(len = length(h$band.h)), region = character(len = length(h$band.h)), exp = numeric(length(h$band.h)), slope = numeric(length(h$band.h)))
high$band = unlist(h$band.h)
high$subj = unlist(h$subj.h)
high$region = unlist(h$region.h)
high$exp = unlist(h$exp.h)
high$slope = unlist(h$slope)

low = data.frame(subj = character(len = length(h$band.h)), band = character(len = length(h$band.h)), region = character(len = length(h$band.h)), exp = numeric(length(h$band.h)), slope = numeric(length(h$band.h)))
low$band = unlist(l$band.l)
low$subj = unlist(l$subj.l)
low$region = unlist(l$region.l)
low$exp = unlist(l$exp.l)
low$slope = unlist(l$slope)

# plot
plot = ggplot(high, aes(x = region, y = exp, fill = band) )
plot + geom_boxplot(notch = FALSE, lwd = 1) + 
  #geom_dotplot(binaxis='y', stackdir='center', dotsize=.5)
  scale_fill_manual(values = wes_palette("Moonrise1",4)) + 
  labs(x = 'Region', y = 'Within Lobe Exp')  + theme_minimal()
ggsave(paste(sens, '_high_exp', '.png', sep = ''))

# plot
plot = ggplot(low, aes(x = region, y = exp, fill = band) )
plot + geom_boxplot(notch = FALSE, lwd = 1) + 
  #geom_dotplot(binaxis='y', stackdir='center', dotsize=.5)
  scale_fill_manual(values = wes_palette("Moonrise1",4)) + 
  labs(x = 'Region', y = 'Within Lobe Exp')  + theme_minimal()
...
# stats
fit1 = lm(exp ~ band+region, high)
Anova(fit1)
summary(fit1)

fit2 = lm(exp ~ band+region, low)
Anova(fit2)
summary(fit2)
...
h_ts = readMat(paste('data/wpli/', sens, '/ts_mod_exp_high.mat', sep = ''))
l_ts = readMat(paste('data/wpli/', sens, '/ts_mod_exp_low.mat', sep = ''))

high_ts = data.frame(subj = character(len = length(h_ts$band.h)), band = character(len = length(h_ts$band.h)), region = character(len = length(h_ts$band.h)), exp = numeric(length(h_ts$band.h)), slope = numeric(length(h$band.h)))
high_ts$band = unlist(h_ts$band.h)
high_ts$subj = unlist(h_ts$subj.h)
high_ts$region = unlist(h_ts$region.h)
high_ts$exp = unlist(h_ts$exp.h)
high_ts$slope = unlist(h_ts$slope)

low_ts = data.frame(subj = character(len = length(h_ts$band.h)), band = character(len = length(h_ts$band.h)), region = character(len = length(h_ts$band.h)), exp = numeric(length(h_ts$band.h)), slope = numeric(length(h$band.h)))
low_ts$band = unlist(l_ts$band.l)
low_ts$subj = unlist(l_ts$subj.l)
low_ts$region = unlist(l_ts$region.l)
low_ts$exp = unlist(l_ts$exp.l)
low_ts$slope = unlist(l_ts$slope)

# plot
plot = ggplot(high_ts, aes(x = region, y = log(exp), fill = band) )
plot + geom_boxplot(notch = FALSE, lwd = 1) + 
  #geom_dotplot(binaxis='y', stackdir='center', dotsize=.5)
  scale_fill_manual(values = rev(brewer.pal(3,'Greys'))) + 
  labs(x = 'Region', y = 'Exp')  + theme_minimal()
ggsave(paste(sens, '_high_ts_exp', '.png', sep = ''))

# plot
plot = ggplot(low_ts, aes(x = region, y = log(exp), fill = band) )
plot + geom_boxplot(notch = FALSE, lwd = 1) + 
  #geom_dotplot(binaxis='y', stackdir='center', dotsize=.5)
  scale_fill_manual(values = rev(brewer.pal(3,'Greys'))) + 
  labs(x = 'Region', y = 'Exp')  + theme_minimal()
ggsave(paste(sens, '_low_ts_exp', '.png', sep = ''))
...
for (i in unique(high$region)){
  for (j in unique(high$band)){
    ...
    plot = ggplot(curr, aes(x = log(exp), y = slope))  
    plot + geom_point(size = 6, color = "slateblue") + labs(x = 'log(Behavioral Expression)', y = 'Slope') + 
      geom_smooth(method="lm", color = 'black') + theme_minimal()
    ggsave(paste(sens, '_high_', i, '_', j, '.png', sep = ''))
    
    curr_stat = cor.test(curr$slope, log(curr$exp), method = 'spearman')
    ...
    plot = ggplot(curr_l, aes(x = log(exp), y = slope))  
    plot + geom_point(size = 6, color = "thistle") + labs(x = 'log(Behavioral Expression)', y = 'Slope') + 
      geom_smooth(method="lm", color = 'black') + theme_minimal() 
    ggsave(paste(sens, '_low_', i, '_', j, '.png', sep = ''))
    
    curr_stat_l = cor.test(curr_l$slope, log(curr_l$exp), method = 'spearman')
...
# stats
stat = lm(slope ~ log(exp)*band*region, data=high)
Anova(stat)
summary(stat)

stat = lm(slope ~ log(exp)*band*region, data=low)
Anova(stat)
summary(stat)
...
stat = lmp(log(exp) ~ region + model + band, data=all)
Anova(stat)
anova(stat)
summary(stat)
...
#plot
plot = ggplot(beta_H, aes(x = region, y = log(exp), fill = model) )
plot + geom_violin(position = dodge, trim = FALSE) + geom_boxplot(position = dodge, width = 0.2, outlier.color = NA) +
  ...
  labs(x = 'Region', y = 'Exp')  + theme_minimal()
ggsave(paste(sens, '_', curr_band, '_H_exp', '.png', sep = ''))

stat1 = lmp(log(exp) ~ as.factor(region) + as.factor(model), data = beta_H)
stat2 = lmp(log(exp) ~ region, data = beta_H)
summary(stat1)
Anova(stat1)
anova(stat1)
...
#plot
plot = ggplot(beta_L, aes(x = region, y = log(exp), fill = model) )
plot + geom_boxplot(notch = FALSE, lwd = 1) + 
  #geom_dotplot(binaxis='y', stackdir='center', dotsize=.5)
  scale_fill_manual(values = (brewer.pal(3,'YlGn'))) + 
  labs(x = 'Region', y = 'Exp')  + theme_minimal()
ggsave(paste(sens, '_', curr_band, '_L_exp', '.png', sep = ''))
