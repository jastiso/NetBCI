...
# load data
Sp = readMat(paste('data/Sp_coeff.mat'))
...
plot = ggplot(plot_data, aes(x = cond, y = Sp, col = cond, fill = band) )
...
stat = permTS(filter(plot_data, cond == "high BE")$Sp, filter(plot_data, cond == "other BE")$Sp)
stat
