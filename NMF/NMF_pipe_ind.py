...
#%% Load data 
    ...
    data = io.loadmat(''.join([data_dir, 'ind_gc_', band, '_', eType, '_', subj, '.mat']))
...
#%% Use the best parameters
    ...
    np.save("".join([save_dir,'ind_gc_', band, '_params']), opt_param)
...
#%% Consensus clustering for best parameters
    ...
    np.save("".join([save_dir, 'ind_gc_',band, '_subset']), subset)
    np.save("".join([save_dir,'ind_gc_', band, '_coeff']), coeff)
    np.save("".join([save_dir, 'gc_',band, '_err']), err)
