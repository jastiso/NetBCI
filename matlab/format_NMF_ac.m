        ...
        for i = subjs
            ...
            for j = 1:numel(sessions)
                ...
                for k = 1:numel(condition)
                    cond = condition{k};
                    load([top_dir, sess, '/', cond, '/', subj, '/FCmatrices/NMF_', freq, '_', eType, '_ac.mat']);
                    if strcmp(freq, 'all')
            ...
            save([save_dir, 'ac_', freq, '_', eType, '_', subj], 'A')
        end
    end
end
    ...
    for f  = 1:numel(bands)
        ...
        for i = subjs
            ...
            for j = 1:numel(sessions)
                ...
                for k = 1:numel(condition)
                    cond = condition{k};
                    load([top_dir, sess, '/', cond, '/', subj, '/FCmatrices/NMF_', freq, '_', eType, '_bl_ac.mat']);
                    if strcmp(freq, 'all')
                        eval(['A = [A, ac_', num2str(eType), '_bl];']);
                    else
                        eval(['A = [A; ac_', num2str(eType), '_bl];']);
                    end
                end
            ...
            save([save_dir, 'ac_bl_', freq, '_', eType, '_', subj], 'A')
        end
    end
end
