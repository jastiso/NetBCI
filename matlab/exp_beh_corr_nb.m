...
% checking relationships to learning for nb NMF
...
for j = 1:numel(sensors)
    ...
    for i = Subj
        ...
        for k = 1:numel(bands)
            ...
            % get subgraph data
            subset = readNPY([data_dir, subj,  '/', sens, '/nb_gc_', f, '_subset.npy']);
            coeff = readNPY([data_dir, subj,  '/', sens, '/nb_gc_', f, '_coeff.npy']);

            
            % get corr
            max_corrs(idx,k,j) = max(corr(behavior_all{i}', coeff'));
            min_corrs(idx,k,j) = min(corr(behavior_all{i}', coeff'));
            figure(1); clf
    ...
    save([R_dir, 'max_exp_b_corr_nb.mat'], 'max_corrs')
    save([R_dir, 'min_exp_b_corr_nb.mat'], 'min_corrs')
    ...
    xlabel('Frequency')
    ylabel('Subject')
    saveas(gca, [save_dir, 'min_behavior_exp_corr_nb.png'], 'png')
    
    % plot realtionship between slope of learning and expression
    for i = 1:numel(bands)
        corrplot([betas(Subj)', improvement(Subj)', max_corrs(:,i), min_corrs(:,i)], 'Var', {'Beta', 'Im', 'Max', 'min'}, 'type', 'pearson','testr', 'on')
        pause(0.001)
    end
    
    figure(1); clf
    corrplot([repmat(betas(Subj)',numel(bands),1), repmat(improvement(Subj)',numel(bands),1),  ...
        reshape(max_corrs,[],1), reshape(min_corrs,[],1)], 'Var',...
        {'Beta',  'Im', 'Max', 'Min'}, 'type', 'spearman','testr', 'on')
    
    % save to R_dir
    save([R_dir, 'exp_beh_cor_nb.mat'], 'betas',  'max_exp', 'sum_exp', 'sd_exp');
...
% figure(1); clf
% s1 = plot(betas(Subj)', max_corrs(:,1), 'k.', 'color', [.7,.7,.7]);
...
% saveas(gca, [save_dir, 'max_beta_alpha_nb.png'], 'png')
% 
% figure(1); clf
...
% saveas(gca, [save_dir, 'sd_beta_alpha_nb.png'], 'png')
% 
% figure(1); clf
...
% saveas(gca, [save_dir, 'max_beta_beta_nb.png'], 'png')
% 
% figure(1); clf
...
% saveas(gca, [save_dir, 'sd_beta_beta_nb.png'], 'png')
