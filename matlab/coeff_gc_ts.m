...
for j = 1:numel(sensors)
    ...
    for i = Subj
        ...
        for k = 1:numel(bands)
            ...
            % get subgraph data
            subset = readNPY([data_dir, subj, '/', sens, '/ts_gc_', f, '_subset.npy']);
            coeff = readNPY([data_dir, subj, '/',sens, '/ts_gc_', f, '_coeff.npy']);
            b_exp = coeff(:,end);
            [~,bSG] = max(b_exp);
            [~,nbSG] = min(b_exp);
            nSG = size(subset,1);
    ...
    for i = 1:numel(bands)
        ...
        saveas(gca, [save_dir, bands{i}, '_energy_ts.png'], 'png')
    end
    idx = E_exp_corr(:,2)~=0;
    ...
    saveas(gca, [save_dir, 'E_BE_corr_all_ts.png'], 'png')
    
    
    % save data
    save([R_dir_s, 'E_ts.mat'], 'E_high', 'E_low', 'E_other');
    save([R_dir_s, 'E_corr_ts.mat'], 'E_exp_corr', 'ebo', 'eso');
    ...
    for n = 1:numel(bands)
        ...
        saveas(gca, [save_dir, bands{n}, '_entropy_ts.png'], 'png')
    end
    figure(2)
    ...
    saveas(gca, [save_dir, 'H_BE_corr_all_ts.png'], 'png')
    
    % save data
    save([R_dir_s, 'H_ts.mat'], 'H_high', 'H_low', 'H_other');
    save([R_dir_s, 'H_corr_ts.mat'], 'H_exp_corr', 'hbo', 'hso');
    
    % mean
    for n = 1:numel(bands)
        figure(1); clf
        boxplot([M_high(:,n), M_low(:,n), M_other(:,n)], [{'High'}, {'Low'}, {'Other'}])
        saveas(gca, [save_dir, bands{n}, '_mean_ts.png'], 'png')
    end
    figure(2)
    ...
    saveas(gca, [save_dir, 'M_BE_corr_all_ts.png'], 'png')
    
    % save data
    save([R_dir_s, 'M_ts.mat'], 'M_high', 'M_low', 'M_other');
    save([R_dir_s, 'M_corr_ts.mat'], 'M_exp_corr', 'mbo', 'mso');
    
    % skew
    for n = 1:numel(bands)
        figure(1); clf
        boxplot([S_high(:,n), S_low(:,n), S_other(:,n)], [{'High'}, {'Low'}, {'Other'}])
        saveas(gca, [save_dir, bands{n}, '_skew_ts.png'], 'png')
    end
    ...
    saveas(gca, [save_dir, 'S_BE_corr_all_ts.png'], 'png')
    % save data
    save([R_dir_s, 'S_ts.mat'], 'S_high', 'S_low', 'S_other');
    save([R_dir_s, 'S_corr_ts.mat'], 'S_exp_corr', 'sbo', 'sso');
