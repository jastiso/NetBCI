...
for j = 1:numel(sensors)
    ...
    for i = 1:numel(bands)
        figure(1); clf
        boxplot([St_high(:,i), St_low(:,i), St_other(:,i)], [{'High'}, {'Behavior'}, {'Other'}])
        saveas(gca, [save_dir, bands{i}, '_sparsity_pr.png'], 'png')
    end
    
    % save data
    save([R_dir_s, 'pr_St.mat'], 'St_high', 'St_low', 'St_other');
    ...
    for n = 1:numel(bands)
        figure(1); clf
        boxplot([S_high(:,n), S_low(:,n), S_other(:,n)], [{'High'}, {'Low'}, {'Other'}])
        saveas(gca, [save_dir, bands{n}, '_skew_sg_pr.png'], 'png')
    end

    % save data
    save([R_dir_s, 'pr_Ssg.mat'], 'S_high', 'S_low', 'S_other');
...
for j = 1:numel(sensors)
    ...
    for i = 1:numel(bands)
        figure(1); clf
        boxplot([St_high(:,i), St_low(:,i), St_other(:,i)], [{'High'}, {'Behavior'}, {'Other'}])
        saveas(gca, [save_dir, bands{i}, '_sparsity_ind.png'], 'png')
    end
    ...
    for n = 1:numel(bands)
        figure(1); clf
        boxplot([S_high(:,n), S_low(:,n), S_other(:,n)], [{'High'}, {'Low'}, {'Other'}])
        saveas(gca, [save_dir, bands{n}, '_skew_sg_ind.png'], 'png')
    end
