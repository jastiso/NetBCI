...
% this script if for unimodal matrices
...
for i = subjs
    ...
    for j = 1:numel(sensors)
        ...
        for k = 1:numel(bands)
            ...
            err = readNPY([data_dir, subj,  '/', sens, '/nb_gc_', f, '_err.npy']);
            % get mag index
            labels = [];
            try
            ...
            for n = 1:nSG
                ...
                dlmwrite([save_dir, f, '_graph_', num2str(n), '_nb.edge'], node_exp, '\t')
                ...
                saveas(gca, [img_dir, f, '_', num2str(n), '_nb.png'], 'png')
                
                % sidebar - make node file
                bv_label = textscan(fid, '%d %10f %10f %d %d %s', 'Delimiter', '\n');
                fclose(fid);
                write_bv_node([save_dir, f, '_graph_', num2str(n), '_nb.node'], bv_label{:,2},bv_label{:,3},double(bv_label{:,4}), 1, sg_deg);
            end
