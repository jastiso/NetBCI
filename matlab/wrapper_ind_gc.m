function [ errors ] = wrapper_ind_gc( session, condition, subjs, data_dir, raw_dir, top_dir, bands, freqs, bl_st, bl_en, dt, lag, t0, st, en, nNode )
% helper function for paralelizing pipeline
% this function is the same as wrapper_pr_gc except it uses independent
% instea of uniform phase randomization
errors = {};
cnte = 1;
    ...
    for k = subjs
        ...
        try
            ...
            for f = 1:numel(bands)
                ...
                saveas(gca, [img_dir, bands{f}, 'pow_ind.png'], 'png')
                ...
                for t = 1:nTrial
                    ...
                    %if t == nTrial
                    %    saveas(gca, [img_dir, bands{f}, 'ind_gc_mag.png'], 'png')
                    %end
                    % save
                    gc_mag(:,t) = GC;
                    ...
                    imagesc(tmp); colorbar; pause(0.001)
                    if t == nTrial
                        saveas(gca, [img_dir, bands{f}, 'ind_gc_grad.png'], 'png')
                    end
                    % save
                    gc_grad(:,t) = GC;
                end
                ...
                save([save_dir, 'NMF_', curr_band, '_mag_ind.mat'], 'gc_mag')
                save([save_dir, 'NMF_', curr_band, '_grad_ind.mat'], 'gc_grad')
            end
        catch
