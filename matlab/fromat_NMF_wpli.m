        ...
        for i = subjs
            try
                ...
                for j = 1:numel(sessions)
                    ...
                    for k = 1:numel(condition)
                        cond = condition{k};
                        load([top_dir, sess, '/', cond, '/', subj, '/FCmatrices/NMF_', freq, '_', eType, '_wpli_ind.mat']);
                ...
                behavior = behavior.*sf;
                A = [A; behavior];
                save([save_dir, 'ind_wpli_', freq, '_', eType, '_', subj], 'A')
            catch
                cnte = cnte + 1;
                errors_ind{cnte} = [top_dir, sess, '/', cond, '/', subj, '/FCmatrices/NMF_', freq, '_', eType, '_ind_wpli.mat'];
            end
        end
    end
end
