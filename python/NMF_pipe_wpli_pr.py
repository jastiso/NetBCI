...
#%% Load data 
    ...
    data = io.loadmat(''.join([data_dir, 'pr_wpli_', band, '_', eType, '_', subj, '.mat']))
...
#%% Use the best parameters
    ...
    np.save("".join([save_dir,'wpli_pr_', band, '_params']), opt_param)
...
#%% Consensus clustering for best parameters
    ...
    np.save("".join([save_dir, 'wpli_pr_',band, '_subset']), subset)
    np.save("".join([save_dir,'wpli_pr_', band, '_coeff']), coeff)
    np.save("".join([save_dir, 'wpli_pr_',band, '_err']), err)
...
#%% Load data 
    ...
    data = io.loadmat(''.join([data_dir, 'wpli_pr_', band, '_', eType, '_', subj, '.mat']))
    val = np.transpose(np.array(data['A']))
    nWin = val.shape[0]
