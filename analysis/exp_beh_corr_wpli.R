...
predictors = c('max', 'max2', 'max3', 'max4', 'min','min2')


# save p_values
stats = data.frame(p_val = rep(0, times = 12), b_val = rep(0, times = 12), pred = c(predictors, 
                                                                                    predictors), 
...
# max
# there must be abetter way to do this besides copying and pasting but lmp doesnt take strings as formulas...
fit1 = lm.beta(lmp(slope ~ max + band, data = corr_data))
summary(fit1)
  
stats$b_val[1] = summary(fit1)$coefficients[2,2]
stats$p_val[1] = summary(fit1)$coefficients[2,5]


# second max
fit2 = lm.beta(lmp(slope ~ max2 + band, data=  corr_data))
summary(fit2)

stats$b_val[2] = summary(fit2)$coefficients[2,2]
stats$p_val[2] = summary(fit2)$coefficients[2,5]

# third max
fit3 = lm.beta(lmp(slope ~ max3 + band, data=  corr_data))
summary(fit3)

stats$b_val[3] = summary(fit3)$coefficients[2,2]
stats$p_val[3] = summary(fit3)$coefficients[2,5]

# 4th max
fit4 = lm.beta(lmp(slope ~ max4 + band, data=  corr_data))
summary(fit4)

stats$b_val[4] = summary(fit4)$coefficients[2,2]
stats$p_val[4] = summary(fit4)$coefficients[2,5]


# min
fit5 = lm.beta(lmp(slope ~ min + band, data=  corr_data))
summary(fit5)

stats$b_val[5] = summary(fit5)$coefficients[2,2]
stats$p_val[5] = summary(fit5)$coefficients[2,5]

# 2nd min
fit6 = lm.beta(lmp(slope ~ min2 + band, data=  corr_data))
summary(fit6)

stats$b_val[6] = summary(fit6)$coefficients[2,2]
stats$p_val[6] = summary(fit6)$coefficients[2,5]
...
# stats
#  max
fit7 = lm.beta(lmp(slope ~ max + band, data=  corr_data_pr))
summary(fit7)

stats$b_val[7] = summary(fit7)$coefficients[2,2]
stats$p_val[7] = summary(fit7)$coefficients[2,5]

# 2nd max
fit8 = lm.beta(lmp(slope ~ max2 + band, data=  corr_data_pr))
summary(fit8)

stats$b_val[8] = summary(fit8)$coefficients[2,2]
stats$p_val[8] = summary(fit8)$coefficients[2,5]

# third
fit9 = lm.beta(lmp(slope ~ max3 + band, data=  corr_data_pr))
summary(fit9)

stats$b_val[9] = summary(fit9)$coefficients[2,2]
stats$p_val[9] = summary(fit9)$coefficients[2,5]

# fourth
fit10 = lm.beta(lmp(slope ~ max4 + band, data=  corr_data_pr))
summary(fit10)

stats$b_val[10] = summary(fit10)$coefficients[2,2]
stats$p_val[10] = summary(fit10)$coefficients[2,5]

# min
fit11 = lm.beta(lmp(slope ~ min + band, data=  corr_data_pr))
summary(fit11)

stats$b_val[11] = summary(fit11)$coefficients[2,2]
stats$p_val[11] = summary(fit11)$coefficients[2,5]

# min2
fit12 = lm.beta(lmp(slope ~ min2 + band, data=  corr_data_pr))
summary(fit12)

stats$b_val[12] = summary(fit12)$coefficients[2,2]
stats$p_val[12] = summary(fit12)$coefficients[2,5]
...
boot_stats = data.frame(p_val = rep(0, times = 12*nBoot), b_val = rep(0, times = 12*nBoot), 
                        pred = rep(c(predictors,predictors), times = nBoot), 
...
for (i in 1:nBoot){
  ...
  #1
  curr_1 = lm.beta(lmp(slope ~ max + band, data=  bootdata[[i]]))
  boot_stats$b_val[cnt] = summary(curr_1)$coefficients[2,2]
  boot_stats$p_val[cnt] = summary(curr_1)$coefficients[2,5]
  cnt = cnt + 1
  
  #2
  curr_2 = lm.beta(lmp(slope ~ max2 + band, data=  bootdata[[i]]))
  boot_stats$b_val[cnt] = summary(curr_2)$coefficients[2,2]
  boot_stats$p_val[cnt] = summary(curr_2)$coefficients[2,5]
  cnt = cnt + 1
  
  #3
  curr_3 = lm.beta(lmp(slope ~ max3 + band, data=  bootdata[[i]]))
  boot_stats$b_val[cnt] = summary(curr_3)$coefficients[2,2]
  boot_stats$p_val[cnt] = summary(curr_3)$coefficients[2,5]
  cnt = cnt + 1
  
  #4
  curr_4 = lm.beta(lmp(slope ~ max4 + band, data=  bootdata[[i]]))
  boot_stats$b_val[cnt] = summary(curr_4)$coefficients[2,2]
  boot_stats$p_val[cnt] = summary(curr_4)$coefficients[2,5]
  cnt = cnt + 1
  
  #low
  curr_min = lm.beta(lmp(slope ~ min + band, data=  bootdata[[i]]))
  boot_stats$b_val[cnt] = summary(curr_min)$coefficients[2,2]
  boot_stats$p_val[cnt] = summary(curr_min)$coefficients[2,5]
  cnt = cnt + 1
  
  #l2
  curr_min2 = lm.beta(lmp(slope ~ min2 + band, data=  bootdata[[i]]))
  boot_stats$b_val[cnt] = summary(curr_min2)$coefficients[2,2]
  boot_stats$p_val[cnt] = summary(curr_min2)$coefficients[2,5]
  cnt = cnt + 1
}
# for for null data
for (i in 1:nBoot){
  ...
  #1
  curr_1 = lm.beta(lmp(slope ~ max + band, data=  bootdata_upr[[i]]))
  boot_stats$b_val[cnt] = summary(curr_1)$coefficients[2,2]
  boot_stats$p_val[cnt] = summary(curr_1)$coefficients[2,5]
  cnt = cnt + 1
  
  #2
  curr_2 = lm.beta(lmp(slope ~ max2 + band, data=  bootdata_upr[[i]]))
  boot_stats$b_val[cnt] = summary(curr_2)$coefficients[2,2]
  boot_stats$p_val[cnt] = summary(curr_2)$coefficients[2,5]
  cnt = cnt + 1
  
  #3
  curr_3 = lm.beta(lmp(slope ~ max3 + band, data=  bootdata_upr[[i]]))
  boot_stats$b_val[cnt] = summary(curr_3)$coefficients[2,2]
  boot_stats$p_val[cnt] = summary(curr_3)$coefficients[2,5]
  cnt = cnt + 1
  
  #4
  curr_4 = lm.beta(lmp(slope ~ max4 + band, data=  bootdata_upr[[i]]))
  boot_stats$b_val[cnt] = summary(curr_4)$coefficients[2,2]
  boot_stats$p_val[cnt] = summary(curr_4)$coefficients[2,5]
  cnt = cnt + 1
  
  #min
  curr_min = lm.beta(lmp(slope ~ min + band, data=  bootdata_upr[[i]]))
  boot_stats$b_val[cnt] = summary(curr_min)$coefficients[2,2]
  boot_stats$p_val[cnt] = summary(curr_min)$coefficients[2,5]
  cnt = cnt + 1
  
  #l2
  curr_min2 = lm.beta(lmp(slope ~ min2 + band, data=  bootdata_upr[[i]]))
  boot_stats$b_val[cnt] = summary(curr_min2)$coefficients[2,2]
  boot_stats$p_val[cnt] = summary(curr_min2)$coefficients[2,5]
  cnt = cnt + 1
}
